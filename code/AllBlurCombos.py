...
for blurType in blurTypes: # multiple types of blur
    ...
    for i in range(7): #7 levels of blur: 1, 3, 7, 15, 31, 63, 127
        ...
        for j in range(7):
            correct = 0
            total = 0
            newTestSet = torchvision.datasets.ImageFolder(root=data_dir + blurType + '/imagenette-320_' + str(tempMult) + '/val',
                                                    transform=transform)
...
y = allAllAccs[0][2]
plt.plot(x, y, styles[2], label = 'Trained on Blur Size 7 px')
y = allAllAccs[0][3]
plt.plot(x, y, styles[3], label = 'Trained on Blur Size 15 px')
y = allAllAccs[0][4]
plt.plot(x, y, styles[4], label = 'Trained on Blur Size 31 px')
y = allAllAccs[0][5]
plt.plot(x, y, styles[5], label = 'Trained on Blur Size 63 px')
y = allAllAccs[0][6]
plt.plot(x, y, styles[6], label = 'Trained on Blur Size 127 px')

plt.legend()
plt.xlabel("Window Size (px)")
plt.ylabel("Percent Accuracy")
plt.title(str(modelType) +  " Classifying Blurred Images After Training on Blurred Images")
plt.ylim(0, 100)
plt.show()
plt.savefig('/u/erdos/cnslab/CNNblur/figures/' + str(modelType) +  ' Classifying Blurred Images After Training on Blurred Images.png')